/* A SIMPLE MSF SIGNAL SIMULATOR FOR THE ARDUINO
 ...
 This is a "Thrown together" sketch designed to work rather
 than look pretty so, please accept it for wht it is!
 
 (C) 2016 by Phil Morris <www.lydiard.plus.com>
 ...
 The MSF signal carries UTC (Universal Time Coordinated) data which is generated by extremely
 precise atomic clocks. UT1 time is derived from observations and measurement of the Earth's
 rotation which is not precise and varies. Therefore, UTC time and UT1 time go out of sync.
 
 The difference between the UTC time and UT1 time is transmitted as DUT1 values which are either
 positive or negative in the range of 0 - 800ms. When the difference becomes too great UTC time
 uses Leap Seconds to correct its time data and these are added or subtracted from time
 to time to keep UTC and UT1 time within acceptable limits.
 
 DUT1 is calculated thus: DUT1 = UT1 − UTC

 The msfTime library reports the DUT1 values but does not do anything with them. For all
 normal uses, the DUT1 values can be ignored. They are included here for demonstration purposes only.
 
 If you feel the need to live your life by UT1 time, it is possible to use the DUT1 values to amend the
 UTC time received from the MSF transmitter.
 
 Only one DUT segment can be set, either DUT_POSITIVE or DUT_NEGATIVE; NOT BOTH!
 Starting with bit "7", the DUT1 values are calculated like this:
 
 DUT Bits	 Value in milliseconds
 --------	 ---------------------
 76543210    
 00000000    0
 10000000    100
 ...
 PLEASE NOTE: THE OUTPUT PULSES GENERATED BY THIS SKETCH ARE PRECISE AND IT MAY
 BE NECESSARY TO ADD AT LEAST 10ms PADDING TO THE msfTime CONFIGURATION IN THE
 RECEIVING SKETCH! SYMPTOMS ARE MISSING SECONDS FROM THE STREAM OR ERATIC RECEPTION!
 */
...
uint8_t segmentLength[9] = {
  8,8,8,5,6,3,6,7,8};
const char * const segmentNames[9] ={
...
{
  Serial.begin(115200);
  // set the Time library to the specified time
...
void loop() 
{
  ...
  while(millis() - startMillis < 500 + PULSE_PADDING);
  digitalWrite(PIN_CARRIER,C_ON);
...
void sendSegment(uint8_t numBits, uint8_t aSegment, uint8_t bSegment)
{
  ...
  for(uint8_t y = 0;y<numBits;y++)
  {
    ...
    case 0:
      // a 100 mS pulse
      while(millis() - startMillis < 110);
      break;
    case 1:
      // a 200 mS pulse
      while(millis() - startMillis < 200 + PULSE_PADDING);
      break;
    case 2:
      // a 100 mS pulse + 100 mS of Carrier + 100 mS pulse
      while(millis() - startMillis < 100 + PULSE_PADDING);
      digitalWrite(PIN_CARRIER,C_ON);
      while(millis() - startMillis < 200 + PULSE_PADDING);
      digitalWrite(PIN_CARRIER,C_OFF);
      while(millis() - startMillis < 300 + PULSE_PADDING);
      break;
    case 3:
      // a 300 mS pulse
      while(millis() - startMillis < 300 + PULSE_PADDING);
