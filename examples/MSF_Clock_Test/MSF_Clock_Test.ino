 ...
 *  intended to aid diagnosis of MSF receiving problems. The accompanying
 ...
 *  guaranteed to be available 24/7/365 so, it would be better to use a RTC (Real Time Clock)
 *  as your time source and synchronise it with the MSF time/date at least once per day.
 *
...
#include <Time.h>       // the Arduino Time library
#include <msfTime.h> // MsfTimeLib library
#include <Streaming.h>  // the Streaming template/library
#include "printF.h"

// change these entries to suit your system
/* For the ESP8266 and the Arduino/ESP8266 IDE */
//#define INTERRUPT 13    // example: ESP8266 interrupt 13 on GPIO13

/* For an Arduino board */
#define INTERRUPT 0				// example: Arduino interrupt 0 on UNO pin 2
#define PON_PIN MSF_NO_PIN      // the pin that is used for PON control (-1 = no pin)
#define LED_PIN 13		        // the pin used for an LED (-1 = no pin)

// weekday text
const char* weekDay[8] = {
...
void loop()
{
  ...
  if (msf.NumSeconds != lastNumSeconds)
  {
    ...
    if (msf.NumSeconds > 0)
    {
      ...
      printf("%03u: ",msf.NumSeconds);
      // print the time and date from the Time library
      printf("%02u:%02u:%02u ",hour(),minute(),second());
	  printf("%s %02u %s %02u\r\n",weekDay[weekday()-1],day(),months[month()],year());
    }
    // match the values to prevent repeating the action until they change again
    lastNumSeconds = msf.NumSeconds;
  }
}

void displayTime()		// print the msfTime library rtcBuffer
{
  // the MSF data is in BCD so we need to convert any value for an array lookup to Decimal for diplaying
  // other values are printed out as HEX
  printf("\r\n        MSF Time: %02X:%02X:%02X ",msf.rtcBuffer[MSF_HOUR],msf.rtcBuffer[MSF_MINUTE],msf.rtcBuffer[MSF_SECOND]);
  // display the date
  printf("%s %02X %s 20%02X\r\n",weekDay[msf.rtcBuffer[MSF_DAY]],msf.rtcBuffer[MSF_DATE],months[msf.bcdToDec(msf.rtcBuffer[MSF_MONTH])],msf.rtcBuffer[MSF_YEAR]);
}
