...
int8_t msfTime::begin(uint8_t _intNum, int8_t _padding)
{
	return begin(_intNum, _padding, HIGH, MSF_NO_PIN, MSF_NO_PIN);
}

int8_t msfTime::begin(uint8_t _intNum, int8_t _padding, uint8_t _carrierOff)
{
	return begin(_intNum, _padding, _carrierOff, MSF_NO_PIN, MSF_NO_PIN);
}

int8_t msfTime::begin(uint8_t _intNum, int8_t _padding, uint8_t _carrierOff, int8_t _ponPin)
{
	return begin(_intNum, _padding, _carrierOff, _ponPin, MSF_NO_PIN);
}

int8_t msfTime::begin(uint8_t _intNum, int8_t _padding, uint8_t _carrierOff, int8_t _ponPin, int8_t _ledPin)
{
	// is the interrupt pin requested within available range?
	if(_intNum > MSF_INT_PINS - 1) return MSF_INVALID_PIN;
	msfPin = interruptPins[_intNum];
	// is the interrupt pin a valid interrupt pin?
	if(msfPin < 0) return MSF_NO_PIN;
	return begin(_intNum, msfPin, _padding, _carrierOff, _ponPin, _ledPin);
}

// the full monty startup, user specifies every parameter
int8_t msfTime::begin(uint8_t _intNum, uint8_t _intPin, int8_t _padding, uint8_t _carrierOff, int8_t _ponPin, int8_t _ledPin)
{
	msfPin = _intPin;
	padding = _padding;
	...
	ponPin = _ponPin;
	ledPin = _ledPin;
	if(ledPin >= 0)	pinMode(ledPin, OUTPUT);	// set LED pin to OUTPUT if specified
	if(ponPin >= 0)
	{
...
void msfTime::rxOn(uint8_t _rxOn)
{
	digitalWrite(ponPin, _rxOn);
}

uint8_t msfTime::rxIsOn(void)
{
	return digitalRead(ponPin);	// return the state of the ponPin. LOW = ON
}

void msfTime::msfPulse(void)	// interrupt routine which is called every time the MSF receiver output changes state
{
  ...
  if (pinState == carrierOff)				// pulse or sub-pulse has started, carrier going off
	{
		...
		if(ledPin >= 0)	digitalWrite(ledPin,HIGH);	// turn on LED if designated ledPin >= 0
  ...
  if(pinState != carrierOff)								// pulse end, carrier going on
	{
		...
		if(ledPin >= 0) digitalWrite(ledPin,LOW);					// turn off the LED if designated ledPin >= 0
	}
...
// store the data in the arrays
  if(pulseLength < 5)  // only pass this point if it's not a "Start" pulse e.g. < 500ms/100
	{
...
uint8_t msfTime::getChunk(uint8_t * _buffer, uint8_t _start, uint8_t _numBits)
{
...
bool msfTime::checkParity(uint8_t _start, uint8_t _numBits, uint8_t _parityBitNum)
{
...
bool msfTime::getBit(uint8_t * _buffer, uint8_t _bitPos)
{
...
uint8_t msfTime::bcdToDec(uint8_t _bcd)	// Convert binary coded decimal to normal decimal numbers
{
  return ( (_bcd/16*10) + (_bcd%16) );
}

uint32_t msfTime::freeMem(void)
{
// report the free DRAM available for sketches
#ifdef ESP_H || defined ESP8266
