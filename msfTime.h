...
ESP8266 GPIO6 thru 11 are reserved for the SPI Flash memory device.

MSF_INT_PINS determines the size of the array, An entry of -1 denotes invalid pin.
All digital pin numbers are the Arduino "Dx" pin numbers or ESP8266 GPIO numbers.
*/

// lookup arrays to convert Arduino Interrupt numbers to physical pins
	...
	static const int8_t interruptPins[MSF_INT_PINS] = {2,3,21,20,19,18};
	...
	static const int8_t interruptPins[MSF_INT_PINS] = {10,11,2};	// based on the Mighty1284P standard pins_arduino.h file
	...
	static const int8_t interruptPins[MSF_INT_PINS] = {0,1,2,3,4,5,-1,-1,-1,-1,-1,-1,12,13,14,15,-1};
#else
	#define MSF_BOARD_TYPE 			"NOT APPLICABLE"
	#define MSF_AVR_TYPE 			"UNDEFINED"
	#define MSF_INT_PINS 			0
	static const int8_t interruptPins[MSF_INT_PINS] = {};
#endif
...
#define MSF_INVALID_PIN -2			// used to return an invalid pin error
...
class msfTime
{
	...
	public:
		...
		int8_t begin(uint8_t _intNum, uint8_t _intPin, int8_t _padding, uint8_t _carrierOff, int8_t _ponPin, int8_t _ledPin);
